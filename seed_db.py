...
def seed_db():
  df = pd.read_csv("university_mental_health_iot_dataset.csv", parse_dates=["timestamp"])
  ...
  for _, row in daily_avg.iterrows():
      insight = DailyInsight(date=row["date"], avg_stress=row["stress_level"], notes=row["notes"])
